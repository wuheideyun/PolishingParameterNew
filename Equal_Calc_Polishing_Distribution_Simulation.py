...
import time as te
'''
def simulation_plot(v1,v2,t1,t2,a,between,num,R,mo):
    # 各项参数
    object_matrix,result=inner_calculate(v1,v2,t1,t2,a,between,num,R,mo)
    # 绘图
    fig=plt.figure()
    ax = plt.axes()
    ...
    plt.show()  #  显示函数图像
def inner_calculate(v1,v2,constant_t,motionless_t,a,between,num,R,mo):
    R_small=R-mo            # 磨头小径
    accelerate_t=round(v2/a)
    t1=accelerate_t
    t2=constant_t
    t3=accelerate_t
    t4=motionless_t
    ...
    # 第二段
        ...
        # 第五段
            ...
            x_0=v1*t+R+50
            y_0=0.5*a*t1**2+v2*t2+v2*t3-0.5*a*t3**2-0.5*a*(t-t1-t2-t3-t4)**2+R+50
        # 第六段
        elif t>=(t1+t2+t3+t4+t5) and t<(t1+t2+t3+t4+t5+t6):
            x_0=v1*t+R+50
            y_0=0.5*a*t1**2+v2*t2+v2*t3-0.5*a*t3**2-0.5*a*t5**2-v2*(t-t1-t2-t3-t4-t5)+R+50
        # 第七段
        elif t>=(t1+t2+t3+t4+t5+t6) and t<(t1+t2+t3+t4+t5+t6+t7):
            x_0=v1*t+R+50
            y_0=0.5*a*t1**2+v2*t2+v2*t3-0.5*a*t3**2-0.5*a*t5**2-v2*t6-v2*(t-t1-t2-t3-t4-t5-t6)+0.5*a*(t-t1-t2-t3-t4-t5-t6)**2+R+50
        # 第八段
    ...
    for i in range(0,n):
        H_mid[:,i*mid_period:i*mid_period+sin_period]=H_mid[:,i*mid_period:i*mid_period+sin_period]+H_period
    # 多磨头叠加
