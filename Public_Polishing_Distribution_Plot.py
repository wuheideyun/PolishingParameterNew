...
def polishing_distribution_Plot(object_matrix,ceramic):
    # 设置权重操作
    ...
    # 绘制矩形线框
    ceramic_width = float(ceramic) * 0.1
    width, length = np.shape(object_matrix)
    x_begin = 0
    y_begin = (width - ceramic_width) / 2
    rect = Rectangle((x_begin, y_begin), length-1, ceramic_width, edgecolor='red', linestyle='--', linewidth=2,
                         fill=False)
    ax.add_patch(rect)
    plt.show()  # 显示函数图像