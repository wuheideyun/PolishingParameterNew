...
# ——————————————出图程序（主程序）———————————————
def data_figure_plot(v1,stay_time,ceramic_width,between,beam_between,R,a,mo,num_input,group):
    # 抛磨量分布子程序
    result = self_define_calculate(v1,stay_time,ceramic_width,between,beam_between,R,a,num_input,group)
    v2 = result[0,1]
    constant_time =result[0,2]
    stay_time =result[0,3]
    num_output = result[0,4]
    delay_time = result[0,5]
    r_P_d = Polishing_distribution_Thread_order_unequal(v1, v2, constant_time, stay_time, a, between,beam_between, num_input, R, mo,delay_time,group)
    result_P_d_matrix,result_P_d_par = r_P_d.emit()
    # 轨迹中心线分布
    m_l_p = middle_line_plot_self_define_order(v1, v2, constant_time, stay_time, a, num_input, between, beam_between,delay_time,group)
    ...
    period = 4 * accelerate_t + 2 * motionless_t + 2 * constant_time
    ...
    ani_text.set_text('Same_grinding_num=%.0f' % (float(num_output)))
    # 设置坐标轴名称
    ax_2.set_xlabel('Tile feed direction')
    ax_2.set_ylabel('Beam swing direction')
    between_cell = math.floor(between / v1 / msize)  # 间距步长
    beam_between_cell = math.floor(beam_between / v1 / msize)  # 间距步长
    num_two = math.ceil(num_input / 2)
    ...
    plt.show()
# --------------参数计算函数---------------
def self_define_calculate(v1,t2,ceramic_width,between,beam_between,R,a,num,group):
...
def polishing_cal(begin,end,v1,v2,constant_t,stay_t,a,R,mod_rho,mod_theta,mo):
    ...
    return H
class Polishing_distribution_Thread_order_unequal():
    def __init__(self,v1, v2, constant_time, stay_time, a, between,beam_between, num, R, mo,delay_time,group):
        # 变量赋值
        self.v1 = v1
        self.v2 = v2
...
class middle_line_plot_self_define_order():
    def __init__(self, v1, v2, t1, t2, a, num, between, beam_between,delay_time,group):
        # 变量输入
        self.v1 = v1
        self.v2 = v2
        self.constant_t = t1
        ...
        for i in range(0,self.group):
            for j in range(0, num_two):
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + j * self.beam_between - j*self.delay_time*self.v1 + self_delay_distance*i,
                           ...
                           color=color_7[idex], s=1)
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + j * self.beam_between +self.between- j*self.delay_time*self.v1+self_delay_distance*i,
                           single_Y_location[0, 0:self.all_time_n-1],
                           color=color_7[idex], s=1)
                idex = idex + 1
        plt.show()  # 显示函数图像
...
if __name__ == '__main__':
    data_figure_plot(300,1.5,900,600,1900,270,650,140,4,2)