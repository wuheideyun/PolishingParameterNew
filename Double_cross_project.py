...
# ——————————————出图程序（主程序）———————————————
def data_figure_plot(v1,v2,constant_time,stay_time,ceramic_width,between,beam_between,num,R,a,mo):
    # 抛磨量分布子程序
    r_P_d = Polishing_distribution_Thread_cross(v1, v2, constant_time, stay_time, a, between, beam_between, num, R, mo)
    result_P_d_matrix,result_P_d_par = r_P_d.emit()
    # 轨迹中心线分布
    m_l_p = middle_line_plot_cross(v1,v2,constant_time,stay_time,a,num,between,beam_between)
    ...
    # 计算第90百分位的阈值（前10%）
    percentile_85 = np.percentile(object_matrix, 90)
    # 对矩阵中大于等于该阈值的元素乘以0.90
    object_matrix[object_matrix >= percentile_85] *= 0.90
    ...
    cross_size = round((v2/a + constant_t + stay_time + v2/a) / 0.01)
    # all_time_n = math.floor(period / msize) * n
    for i in range(0, num_two):
        if (i + 2) % 2 == 0:
            ax_2.scatter(single_X_location[0, 0:all_time_n - 1] + i * beam_between,
                       single_Y_location[0, 0:all_time_n - 1],
                       color=color_7[i], s=1)
            ax_2.scatter(single_X_location[0, 0:all_time_n - 1] + i * beam_between + between,
                       single_Y_location[0, 0:all_time_n - 1],
                       color=color_7[i], s=1)
        # 横梁数为偶数
        else:
            ax_2.scatter(single_X_location[0, 0:all_time_n - cross_size] + i * beam_between,
                       single_Y_location[0, cross_size - 1:all_time_n - 1],
                       color=color_7[i], s=1)
            ax_2.scatter(single_X_location[0, 0:all_time_n - cross_size] + i * beam_between + between,
                       single_Y_location[0, cross_size - 1:all_time_n - 1],
                       color=color_7[i], s=1)
    plt.show()
# ------------抛磨量分布仿真函数-------------
...
def polishing_cal(begin,end,v1,v2,constant_t,stay_t,a,R,mod_rho,mod_theta,mo):
    ...
    return H
# 抛磨量分布，子工作线程
class Polishing_distribution_Thread_cross():
    def __init__(self,v1, v2, constant_time, stay_time, a, between, beam_between, num, R, mo):
        # 变量赋值
        self.v1 = v1
        self.v2 = v2
...
class middle_line_plot_cross():
    def __init__(self,v1,v2,t1,t2,a,num,between,beam_between):
        # 变量输入
        self.v1 = v1
        self.v2 = v2
        self.constant_t = t1
...
if __name__ == '__main__':
    data_figure_plot(300,400,1,1.5,900,600,1500,6,270,650,140)