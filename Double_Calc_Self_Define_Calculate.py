...
def self_define_calculate(v1,ceramic_width,between,between_beam,R,a,num,group):
    ...
    return result
'''
def self_define_calculate(v1,ceramic_width,between,between_beam,R,a,num):
    B=ceramic_width+200-2*R
    distance_period=between*2
    t_all=round(distance_period/v1,2)
    # 边部停留时间设定
    if ceramic_width >= 1200:
        t2 = 0.5
...
'''
    ...
    t1 = round(t_a_in - 2*t_a,2)
    v2 = round(a * t_a, 2)
    delay_time=round((between_beam-between/num*2)/v1,2)
    result=np.zeros((1,7))
    result[0 , 0] = v1
    result[0 , 1] = v2
    result[0 , 2] = t1
