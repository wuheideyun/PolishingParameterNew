...
class middle_line_plot_equal():
    def __init__(self,v1,v2,t1,t2,a,num,between,between_beam):
        # 变量输入
        self.v1 = v1
        self.v2 = v2
        self.constant_t = t1
    ...
    def figure_plot(self):
        ...
        between_beam_cell = math.floor(self.between_beam / self.v1 / msize)  # 间距步长
        ...
        for i in range(0, num_two):
            ...
            plt.scatter(single_X_location+ i * self.between_beam,single_Y_location,color=color_7[i])
...
class middle_line_plot_cross():
    ...
    def figure_plot(self):
        ...
        #fig.suptitle('磨头中心轨迹曲线')
        ax=fig.add_subplot(111)
        ax.set_xlim((-200,period * 6 * self.v1+self.between))
        ax.set_ylim((-200, self.a * (self.v2 / self.a) ** 2 + self.v2 * self.constant_t + 600))
        ax.set_aspect('equal', adjustable='box')
        # 设置图片文本
        ani_text = ax.text(period * 2 * self.v1,
        ...
        for i in range(0, num_two):
            if (i+2) % 2 == 0:
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + i * self.between_beam,
                single_Y_location[0,0:self.all_time_n-1],
                color=color_7[i])
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + i * self.between_beam + self.between,
                single_Y_location[0,0:self.all_time_n-1],
                color=color_7[i])
            # 横梁数为偶数
            else:
                ax.scatter(single_X_location[0,0:self.all_time_n-self.cross_size] + i * self.between_beam,
                single_Y_location[0,self.cross_size-1:self.all_time_n-1],
                color=color_7[i])
                ax.scatter(single_X_location[0,0:self.all_time_n-self.cross_size] + i * self.between_beam + self.between,
                single_Y_location[0, self.cross_size-1:self.all_time_n-1],
                color=color_7[i])
        plt.show()  # 显示函数图像
    ...
    def figure_plot(self):
        ...
        for i in range(0, num_two):
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + i * self.between_beam - i*self.delay_time*self.v1,
                           single_Y_location[0, 0:self.all_time_n-1],
                           color=color_7[i])
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + i * self.between_beam +self.between- i*self.delay_time*self.v1,
                           single_Y_location[0, 0:self.all_time_n-1],
                           color=color_7[i])
        plt.show()  # 显示函数图像
        ...
        for i in range(0,self.group):
            for j in range(0, num_two):
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + j * self.between_beam - j*self.delay_time*self.v1 + self_delay_distance*i,
                           single_Y_location[0, 0:self.all_time_n-1],
                           color=color_7[i+j])
                ax.scatter(single_X_location[0, 0:self.all_time_n-1] + j * self.between_beam +self.between- j*self.delay_time*self.v1+self_delay_distance*i,
                           single_Y_location[0, 0:self.all_time_n-1],
                           color=color_7[i+j])
        plt.show()  # 显示函数图像