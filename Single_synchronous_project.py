...
# ——————————————出图程序（主程序）———————————————
def data_figure_plot(v1,v2, constant_time, stay_time,num,a,ceramic_width,beam_between,R,mo):
    # 抛磨量分布子程序
    r_P_d = Polishing_distribution_equal(v1, v2, constant_time, stay_time, a,beam_between, num, R, mo)
    result_P_d_matrix,result_P_d_par = r_P_d.emit()
    # 轨迹中心线分布
    m_l_p = middle_line_plot_equal(v1,v2,constant_time, stay_time,a,num,beam_between)
    ...
    for i in range(0, num):
        ax_2.scatter(single_X_location + i * beam_between, single_Y_location,
                    color=color_7[i], s=1)
    plt.show()
# ------------抛磨量分布仿真函数-------------
# 多进程计算函数
def polishing_cal(begin,end,v1,v2,constant_t,stay_t,a,R,mod_rho,mod_theta,mo):
    ...
    return H
class Polishing_distribution_equal():
...
if __name__ == '__main__':
    data_figure_plot(300,500, 1, 0.8,5,650,900,900,270,140)