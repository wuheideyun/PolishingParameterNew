...
# ——————————————出图程序（主程序）———————————————
def data_figure_plot(v1,ceramic_width,beam_between,R,a,stay_time,num_input,group,mo):
    # 抛磨量分布子程序
    result = self_define_calculate(v1,ceramic_width,beam_between,R,a,num_input,group,stay_time)
    v2 = result[0,1]
    constant_time =result[0,2]
    stay_time =result[0,3]
    num_output = math.ceil(result[0,4])
    delay_time = result[0,5]
    between = result[0,8]
    r_P_d = Polishing_distribution_self_order(v1, v2, constant_time, stay_time, a,between,beam_between, num_input, R, mo,delay_time,group)
    result_P_d_matrix,result_P_d_par = r_P_d.emit()
    # 轨迹中心线分布
    m_l_p = middle_line_plot_self_order(v1, v2, constant_time, stay_time, a, num_input, between, beam_between,delay_time,group)
    ...
    ani_text.set_text('Same_grinding_num=%.0f' % (num_output))
    # 设置坐标轴名称
    ax_2.set_xlabel('Tile feed direction')
    ax_2.set_ylabel('Beam swing direction')
    color_7 = ['red', 'orange', 'green', 'cyan', 'blue', 'purple', 'yellow', 'lightgreen', 'slategrey',
               'cornflowerblue', 'navy', 'indigo', 'violet', 'plum', 'oldlace', 'maroon', 'lightcyan', 'lightseagreen',
...
# --------------参数计算函数---------------
# 自定义抛光轨迹算法
def self_define_calculate(v1,ceramic_width,beam_between,R,a,num,group,stay_time):
    # 计算单周期时间
    beam_speed_up = 850
...
def polishing_cal(begin,end,v1,v2,constant_t,stay_t,a,R,mod_rho,mod_theta,mo):
    ...
    return H
class Polishing_distribution_self_order():
...
# ------------- 轨迹中心线分布---------------
class middle_line_plot_self_order():
...
if __name__ == '__main__':
    data_figure_plot(300,900,900,270,650,0.8,3,2,140)