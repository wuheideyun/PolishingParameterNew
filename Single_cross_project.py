...
# ——————————————出图程序（主程序）———————————————
def data_figure_plot(v1,v2,constant_time,stay_time,num,ceramic_width,beam_between,R,a,mo):
    # 抛磨量分布子程序
    r_P_d = Polishing_distribution_cross(v1, v2, constant_time, stay_time, a,beam_between, num, R, mo)
    result_P_d_matrix,result_P_d_par = r_P_d.emit()
    # 轨迹中心线分布
    m_l_p = middle_line_plot_cross(v1,v2,constant_time,stay_time,a,num,beam_between)
    ...
    ax_2.set_xlim((-200, period * 3 * v1 + 3*beam_between))
    ...
    cross_size = round((accelerate_t + constant_t + accelerate_t + motionless_t) / msize)
    ...
    plt.show()
# --------------参数计算函数---------------
def single_num_calculate(v1,ceramic_width,beam_between,R,a):
...
def polishing_cal(begin,end,v1,v2,constant_t,stay_t,a,R,mod_rho,mod_theta,mo):
    ...
    return H
class Polishing_distribution_cross():
    def __init__(self,v1, v2, constant_time, stay_time, a,beam_between, num, R, mo):
        # 变量赋值
        self.v1 = v1
        self.v2 = v2
    ...
    def matrix_cal(self, H_all):
        ...
        for i in range(0, self.num):
            if (i+2) % 2 == 0: # 第 奇数 个横梁
                ...
                all_H[:, beam_between_cell * i:self.c_length_mulcell - 1-mid_period] = (
                        all_H[:, beam_between_cell * i : self.c_length_mulcell-1-mid_period ] +
                        H_mid[:, cross_size : self.c_length_mulcell-1-beam_between_cell*i + cross_size-mid_period ])
        # 计算 抛磨变异系数
        # 如果要计算此模块，周期数必须大于等于3
...
if __name__ == '__main__':
    data_figure_plot(300,500,1,0.8,5,900,600,270,650,150)